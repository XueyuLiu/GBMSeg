...
colors_rgb[:, 1] = np.linspace(0, 255, num_patches, dtype=np.uint8)  # S: 255 (full saturation)
colors_rgb[:, 2] = np.linspace(0, 255, num_patches, dtype=np.uint8)  # V: 255 (full brightness)
...
print(colors_rgb_tuples)
print(type(colors_rgb))
...
for patch1, patch2, mask_color in zip(patch_set1, patch_set2, colors_rgb_tuples):
    ...
    print(mask_color_rgb)
    print(type(mask_color_rgb))
...
""" for i, point in enumerate(patch_set1):
    ...
    # overlay1[y:y+patch_size, x:x+patch_size] = colors_rgb
    image1_np[y:y+patch_size, x:x+patch_size] = np.tile(color_new, (patch_size, patch_size, 1))

for i, point in enumerate(patch_set2):
    x, y = point
    color_new = colors_rgb[i % len(colors_rgb)]
    # color = colors[i % len(colors)]  # 取颜色序列中的颜色
    image2_np[y:y+patch_size, x:x+patch_size] = np.tile(color_new, (patch_size, patch_size, 1))
    # overlay2[y:y+patch_size, x:x+patch_size] = colors_rgb """
...
img2_np = cv2.addWeighted(np.array(images[1]), alpha, overlay2, 1-alpha, 0)
 """
""" img1 = Image.fromarray(image1_np)
img2 = Image.fromarray(image2_np) """
""" img1 = Image.fromarray(overlay1)
img2 = Image.fromarray(overlay2) """


# 保存图片
plt.imsave('/Data20T/data20t/data20t/envs/shigz1/code/dinov2-main/dinov2-main/images/test_0/output/1.png', image1_np)
plt.imsave('/Data20T/data20t/data20t/envs/shigz1/code/dinov2-main/dinov2-main/images/test_0/output/2.png', image2_np)
